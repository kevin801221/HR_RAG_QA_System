...
class EnhancedHRSystem:
    ...
    def get_enhanced_response(self, question: str, context_nodes: List[Any]) -> Tuple[str, List[DocumentSource]]:
        """生成增強的回應和來源資訊"""
        prompt = f"""
        ...
        for node in context_nodes:
            if hasattr(node, 'metadata'):
                source = DocumentSource(
                    title=self.document_sources.get(node.metadata.get('file_path', ''), {}).get('title', 'Unknown'),
